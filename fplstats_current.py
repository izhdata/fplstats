...
header_row = ["Name", "Position", "Club", "Price",
              "Game week","Opposition","Home or Away", "Home team goals",
              "Away team goals","Points","Minutes played",
              "Goals scored","Assists","Clean sheets","Goals conceded",
              "Own goals","Penalties saved","Penalties missed",
              "Yellow cards","Red cards","Saves","Bonus",
              "Bonus Points System","Influence","Creativity","Threat",
              "ICT Index","NT","SB","Price"]
with open('fplstats1920.csv', 'a', newline='', encoding="utf-8") as csvfile:
...
for x in range(18):
    ...
    for i in range(num_players):
        ...
        if not extra_divs:
            ...
            try:
                player_stats = driver.find_element_by_xpath("/html/body/div/div/dialog/div/div[2]/div[2]/div/div/div[1]/div/table/tbody")
            ...
            try:
                player_stats = driver.find_element_by_xpath("/html/body/div/div/dialog/div/div[2]/div[3]/div/div/div[1]/div/table/tbody")
            except NoSuchElementException:
                player_stats = []
            ...
            if not player_stats:
                ...
                if len(player_stats.text) > 0:  
                    for row in player_stats.find_elements_by_css_selector('tr'):
                        ...
                        week = player_data[0]
                        opposition = player_data[1]
                        opposition = opposition.replace("(","-")
                        opposition = opposition.replace(")","-")
                        opposition = opposition.replace(" ","")
                        match_data = list(opposition.split("-"))
                        player_output = player + list(player_data[0]) + match_data + player_data[2:]
